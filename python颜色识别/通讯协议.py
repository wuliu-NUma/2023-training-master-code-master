...
blue_threshold=(10, 35, -2, 41, -54, -19)   #亮处蓝色阈值(10, 35, -2, 41, -54, -19)(31, 78, -16, 16, -39, -8)(11, 63, -5, 36, -52, -35)  (39, 58, -4, 15, -53, -35)(44, 65, -26, 23, -52, -1)(19, 49, 0, 123, -119, -8)
...
target_blob_area3=[500,8000]           #物料捕捉面积最大值与最小值限制，防止捕捉到错误目标
      ...
      #  print("进入模式")
        ...
        while 0  in target_flag:                    #不断捕捉直到标志全为1或超出次数上限
            ...
            for i in range(0,3):
                ...
                if blobs:
                    for b in blobs:
                        if b.w()*b.h()>target_blob_area3[0] and b.w()*b.h()<target_blob_area3[1]:
            ...
            if 0 not  in target_flag:
                ...
                print("上层物料摆放结果如下：")
                xxx1 = store_up[0]*100+store_up[1]*10+store_up[2]
                yyy1 = store_down[0]*100+store_down[1]*10+store_down[2]
                print(xxx1)
                print(yyy1)
                TSTM32.x = xxx1
                TSTM32.y = yyy1
        ...
        if environment=="light":
               ...
               color_temp=color_template_dark
        for i in range(0,3):
            blobs = img.find_blobs([color_temp[i]],roi=(68,0,185,240),merge=True,x_tride=10,y_tride=10,area_threshold=300,pixels_threshold=200,margin=1)
            if blobs:
                for b in blobs:
